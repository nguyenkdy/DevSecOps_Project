...
API="http://k8s-ecommerc-apigatew-afc7d5cc0d-1740156965.ap-southeast-1.elb.amazonaws.com/api/v1"
...
echo ""
echo "======================================================"
echo " EcomShop DEV — Seed dữ liệu sản phẩm"
echo "======================================================"

# ── 1. Đăng ký tài khoản (bỏ qua nếu đã tồn tại) ─────────
echo ""
echo "[1/6] Đăng ký tài khoản $ADMIN_EMAIL ..."

curl -s -X POST "$API/auth/register" \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"$ADMIN_EMAIL\",\"password\":\"$ADMIN_PASSWORD\",\"fullName\":\"Dev Admin\"}" | python3 -c "
import sys, json
r = json.load(sys.stdin)
if 'id' in r:
    print('  Đăng ký thành công:', r.get('email'))
else:
    print('  Tài khoản đã tồn tại hoặc lỗi:', r.get('message', r))
" 2>/dev/null || echo "  (bỏ qua — đã tồn tại)"

sleep 3

# ── 2. Set admin role ────────────────────────────────────
echo ""
echo "[2/6] Cấp quyền admin cho $ADMIN_EMAIL ..."

kubectl run set-admin-dev --image=postgres:17 -n ecommerce-dev --restart=Never \
  --env="PGPASSWORD=$DB_PASS" \
  -- psql -h "$DB_HOST" -U postgres -d user_db_dev \
  -c "UPDATE users SET role = 'admin' WHERE email = '$ADMIN_EMAIL' RETURNING email, role;" \
  2>/dev/null || true

sleep 8
kubectl logs set-admin-dev -n ecommerce-dev 2>/dev/null || true
kubectl delete pod set-admin-dev -n ecommerce-dev 2>/dev/null || true
...
echo "======================================================"
echo ""
echo "Truy cập web dev:"
echo "  http://k8s-ecommerc-frontend-098809d8ea-1169492521.ap-southeast-1.elb.amazonaws.com"
echo ""
